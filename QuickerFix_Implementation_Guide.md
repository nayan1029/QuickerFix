...
## SECTION 19: TCS Interview Preparation

### 19.1 Technical Q&A

**Q: Why did you choose Spring Boot for this project?**
A: Spring Boot eliminates manual configuration through auto-configuration based on the classpath. The embedded Tomcat server means no separate deployment step. Starter dependencies ensure compatible library versions. For a CRUD-heavy platform like QuickerFix with 35+ APIs, Spring Data JPA and Spring Security save thousands of lines of boilerplate code while still giving full control when needed.

**Q: Explain the JWT authentication mechanism in your project.**
A: JWT (JSON Web Token) has three parts: Header (algorithm), Payload (claims: email, role, expiry), and Signature (HMAC-SHA256). When a user logs in, the server creates a JWT signed with a secret key and returns it. The client stores it in localStorage and sends it in every request's Authorization header. Our JwtFilter (OncePerRequestFilter) intercepts each request, extracts the token, validates the signature and expiry, and sets the authentication in SecurityContextHolder. Since tokens are self-contained, no server-side session storage is needed — this is stateless authentication.

**Q: How does the Priority Algorithm prevent older complaints from being ignored?**
A: The age score component adds 2 points per day the report remains open, capped at 20. So a LOW severity complaint (score 5) from 10 days ago gets +20 age bonus = 25 total, which equals a brand new MEDIUM severity complaint. After 15 days it scores 35, beating new MEDIUM complaints. This prevents starvation — no complaint can be ignored indefinitely regardless of initial severity.

**Q: Explain the Haversine formula and why you used it.**
A: The Haversine formula calculates the great-circle (shortest path on Earth's sphere) distance between two GPS coordinates. I use it in two places: (1) At report submission time — query PostgreSQL for reports within 100m radius using the formula embedded in JPQL for DB-level efficiency. Same-category match means it's a duplicate — we merge instead of creating a new work order. (2) In LocationUtils.java for filtering nearby reports in Java when the SQL approach isn't applicable.

**Q: What is the difference between @RestController and @Controller?**
A: `@Controller` returns a view name (for server-side rendering with Thymeleaf/JSP). `@RestController` = `@Controller` + `@ResponseBody` — every method automatically serializes the return value to JSON. For a REST API serving a React frontend, `@RestController` is always used.

**Q: How does @Transactional work and why is it important?**
A: `@Transactional` wraps the method in a database transaction. If any exception occurs within the method, ALL database operations in that method are rolled back atomically. Example: in `assignWorker()`, we save an Assignment AND update the Report status in the same method. If the Report update fails, the Assignment creation is also rolled back. Without `@Transactional`, you could end up with an Assignment created but the Report status not updated — inconsistent data.

**Q: Explain CORS and how you handled it.**
A: CORS (Cross-Origin Resource Sharing) is a browser security mechanism that blocks requests from a different origin (protocol + domain + port). Our React frontend on localhost:3000 calls the backend on localhost:8080 — different ports = different origin = CORS blocked by default. We configured `CorsConfig` to add `Access-Control-Allow-Origin: http://localhost:3000` headers, explicitly allowing the frontend. In production, this would be the actual domain.

**Q: Why did you use DTOs instead of returning JPA entities directly?**
A: 1) Security: User entity has `password` field (BCrypt hash) — returning the entity would expose it. 2) Circular references: Report has List<Attachment>, Attachment has Report — infinite JSON loop. 3) Performance: DTOs include computed fields like `assignedWorkerName` that span multiple entities. 4) Decoupling: DB schema can change without breaking the API contract.

**Q: What is the difference between EAGER and LAZY loading in JPA?**
A: EAGER loading fetches related entities immediately when the parent is loaded (always, even if not used). LAZY loading fetches related entities only when first accessed (on demand). We use `FetchType.LAZY` for collections (OneToMany) to avoid loading thousands of comments/attachments just to show a report list. For mandatory single references (ManyToOne like category or citizen), EAGER is acceptable.

**Q: How would you scale this system if it had 1 million users?**
A: 1) Database indexing on `latitude`, `longitude`, `status`, `category_id` for fast queries. 2) Redis caching for frequent reads (dashboard stats, categories, departments). 3) Message queue (RabbitMQ/Kafka) for notifications instead of synchronous DB writes. 4) Horizontal scaling of backend behind a load balancer. 5) PostgreSQL read replicas for reporting queries. 6) CDN for uploaded images instead of local file storage.

### 19.2 Project Metrics (Mention in Interview)
- **65+ Java source files** across 10 packages
- **11 database tables** with proper JPA relationships
- **35+ REST API endpoints** with role-based access control
- **8-stage complaint lifecycle** with validated state transitions
- **5-factor priority algorithm** (0-100 scoring scale)
- **Haversine geospatial duplicate detection** (100m radius)
- **3 user roles** with Spring Security @PreAuthorize
- **Stateless JWT authentication** (24-hour expiry)
- **16 React components/pages** with Bootstrap 5
- **1-click demo login** for all 3 roles

### 19.3 Resume Keywords for This Project
When describing QuickerFix on your resume:
- *"Developed a full-stack civic complaint management platform with 8-stage lifecycle tracking, role-based access control (CITIZEN/WORKER/ADMIN), JWT authentication, and a custom priority scoring algorithm"*
- *"Implemented Haversine-based geospatial duplicate detection within 100m radius, reducing redundant work orders"*
- *"Designed 35+ RESTful APIs using Spring Boot 3 with Spring Data JPA, Hibernate, and PostgreSQL"*
- *"Built React frontend with Bootstrap 5, protected routes, real-time notifications, and role-specific dashboards"*
...
| Error | Cause | Fix |
|-------|-------|-----|
| `java.lang.ExceptionInInitializerError: TypeTag::UNKNOWN` | Lombok incompatible with Java 22+ | Set JAVA_HOME to Java 21 |
| `Connection refused: localhost:5432` | PostgreSQL not running | Start PostgreSQL service |
| `database "quickerfix_db" does not exist` | DB not created | Run `CREATE DATABASE quickerfix_db;` |
| `cannot infer type arguments for ApiResponse<>` | Using constructor on @Builder class | Use `ApiResponse.success("msg", data)` |
| `CORS error in browser` | Frontend origin not allowed | Check CorsConfig allowed origins |
| `401 Unauthorized` | Missing/expired JWT | Re-login and use fresh token |
| `403 Forbidden` | Valid JWT but wrong role | Check @PreAuthorize role matches user's role |
| `Could not find symbol: findByWorker` | Missing repo method | Add `List<Assignment> findByWorker(User worker)` |
| `npm start` fails | Node modules missing | Run `npm install` first |

---

*Generated: August 2026 | QuickerFix v1.0 | Nayan Kumar Shukla*
