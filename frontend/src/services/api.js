...
api.interceptors.response.use(
  // Backend responses are consistently wrapped as { success, message, data, timestamp }.
  // Consumers therefore receive the payload directly as `res.data`.
...
// Reports
export const createReport = ({ title, description, categoryId, severity, address, lat, lng, photos = [] }) => {
  const formData = new FormData();
  formData.append('request', new Blob([JSON.stringify({
    title, description, categoryId: Number(categoryId), severity, address,
  ...
  return api.post('/reports', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
};
...
export const updateStatus = (id, data) => api.put(`/reports/${id}/status`, { ...data, newStatus: data.newStatus || data.status });
...
export const addComment = (id, data) => api.post(`/reports/${id}/comments`, { ...data, content: data.content || data.text });
